...
TEST(PlayerBotIntegrationTest, MultiBotCoordinationUsesRealLifecycleAndClearsReservations) {
	PlayerBotDatabaseFixture first(g_database()),second(g_database());createWalkableTile(first.start);createWalkableTile(second.start);BotManager manager(g_game());auto a=loginBotOrReport(manager,first.name);auto b=loginBotOrReport(manager,second.name);ASSERT_NE(nullptr,a);ASSERT_NE(nullptr,b);auto pa=std::const_pointer_cast<Player>(a->getPlayer());auto pb=std::const_pointer_cast<Player>(b->getPlayer());
	BotCoordinationPolicy policy{.id=7,.configuredMembers={first.playerId,second.playerId},.configuredLeader=first.playerId,.revision=1};BotCoordinationGroupObservation observed{.id=7,.revision=1,.members={{.id=first.playerId,.sessionGeneration=1,.revision=1,.vocationId=pa->getVocationId(),.position=pa->getPosition(),.configured=true,.playerBot=true},{.id=second.playerId,.sessionGeneration=1,.revision=1,.vocationId=pb->getVocationId(),.position=pb->getPosition(),.configured=true,.playerBot=true}}};auto decision=BotCoordination::evaluate(policy,observed);EXPECT_EQ(first.playerId,decision.leaderId);EXPECT_EQ(2,decision.roles.size());EXPECT_FALSE(decision.directGameplayAction);
	std::vector<BotCoordinationReservation> reservations;EXPECT_EQ(BotCoordinationFailure::None,BotCoordination::reserve(reservations,{.groupId=7,.memberId=first.playerId,.type=BotCoordinationReservationType::LootCorpse,.targetSignature=99,.observationRevision=1,.sessionGeneration=1,.expiresAt=10,.policyRevision=1},policy,0));EXPECT_EQ(BotCoordinationFailure::Conflict,BotCoordination::reserve(reservations,{.groupId=7,.memberId=second.playerId,.type=BotCoordinationReservationType::LootCorpse,.targetSignature=99,.observationRevision=1,.sessionGeneration=1,.expiresAt=10,.policyRevision=1},policy,0));EXPECT_TRUE(manager.logout(first.name,false));BotCoordination::invalidate(reservations,first.playerId);EXPECT_TRUE(reservations.empty());EXPECT_TRUE(manager.logout(second.name,false));ASSERT_TRUE(first.cleanup());ASSERT_TRUE(second.cleanup());
}
