...
TEST(PlayerBotIntegrationTest, FleetManagerOwnsAuthoritativeLoginDrainAndCoordinationCleanup) {
	...
	const auto loginEvents=manager.managedSessionEventsAfter(0);ASSERT_EQ(2,loginEvents.size());EXPECT_EQ(BotManagedSessionAction::Login,loginEvents.front().action);EXPECT_EQ(BotManagedSessionReason::Reconciliation,loginEvents.front().reason);EXPECT_EQ(1,loginEvents.front().sessionGeneration);
	const auto identities=manager.managedSessionIdentities();ASSERT_EQ(2,identities.size());EXPECT_TRUE(std::ranges::all_of(identities,&BotManagedSessionIdentity::authoritativelyPlaced));
	...
	const auto lifecycleEvents=manager.managedSessionEventsAfter(loginEvents.back().sequence);ASSERT_EQ(2,lifecycleEvents.size());EXPECT_TRUE(std::ranges::all_of(lifecycleEvents,[](const auto &event){return event.action==BotManagedSessionAction::Logout&&event.reason==BotManagedSessionReason::Reconciliation;}));
