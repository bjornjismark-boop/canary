...
TEST(PlayerBotTest, ManagedBotSurvivesBeyondNetworkPingTimeoutThreshold) {
	const Player managedBot(PlayerControlType::Bot);
	EXPECT_FALSE(managedBot.usesNetworkPingTimeout());
	EXPECT_FALSE(managedBot.hasNetworkPingTimedOut(600000));
}

TEST(PlayerBotTest, UnresponsiveOrdinaryPlayerStillTimesOut) {
	const Player ordinary(PlayerControlType::Network);
	EXPECT_TRUE(ordinary.usesNetworkPingTimeout());
	EXPECT_FALSE(ordinary.hasNetworkPingTimedOut(59999));
	EXPECT_TRUE(ordinary.hasNetworkPingTimedOut(60000));
}
...
namespace { std::string fleetJson(uint64_t revision=1){return fmt::format(R"({{"schemaVersion":1,"revision":{},"population":{{"enabled":true,"desiredOnline":1,"minimumOnline":0,"maximumOnline":2,"absoluteHardMaximum":2}},"members":[{{"id":1,"name":"one","vocationCategory":1,"allowedRegionIds":[7]}}],"distribution":[{{"dimension":"vocation","category":1,"minimum":0,"desired":1,"maximum":1}}],"schedule":[{{"startMinute":60,"endMinute":120,"desiredOnline":2}}]}})",revision);} }
TEST(PlayerBotFleetConfigurationTest,ValidVersionedConfigurationParses){auto r=BotFleetConfiguration::parse(fleetJson());ASSERT_TRUE(r.success());EXPECT_EQ(1,r.revision->schemaVersion);EXPECT_EQ(1,r.revision->revision);}
TEST(PlayerBotFleetConfigurationTest,UnsupportedSchemaIsRejectedAtomically){auto json=fleetJson();json.replace(json.find("schemaVersion\":1")+15,1,"2");EXPECT_EQ(BotFleetConfigurationFailure::UnsupportedSchema,BotFleetConfiguration::parse(json).failure);}
TEST(PlayerBotFleetConfigurationTest,NegativePopulationIsRejected){auto json=fleetJson();json.replace(json.find("desiredOnline\":1")+15,1,"-1");EXPECT_FALSE(BotFleetConfiguration::parse(json).success());}
TEST(PlayerBotFleetConfigurationTest,SecretBearingKeysAreRejected){auto json=fleetJson();json.insert(json.size()-1,R"(,"apiToken":"hidden")");EXPECT_EQ(BotFleetConfigurationFailure::SecretRejected,BotFleetConfiguration::parse(json).failure);}
TEST(PlayerBotFleetConfigurationTest,OverlappingScheduleIsRejected){auto json=fleetJson();auto pos=json.rfind(']');json.insert(pos,R"(,{"startMinute":100,"endMinute":130,"desiredOnline":1})");EXPECT_EQ(BotFleetConfigurationFailure::InvalidPolicy,BotFleetConfiguration::parse(json).failure);}
TEST(PlayerBotFleetConfigurationTest,ScheduleSelectionIsDeterministic){auto r=BotFleetConfiguration::parse(fleetJson());ASSERT_TRUE(r.success());EXPECT_EQ(2,BotFleetConfiguration::scheduledDesired(*r.revision,90));EXPECT_EQ(1,BotFleetConfiguration::scheduledDesired(*r.revision,30));}
TEST(PlayerBotFleetConfigurationTest,ChangeSummaryIsDeterministic){auto a=BotFleetConfiguration::parse(fleetJson(1));auto b=BotFleetConfiguration::parse(fleetJson(2));ASSERT_TRUE(a.success()&&b.success());EXPECT_EQ(BotFleetConfiguration::summarize(a.revision.get(),*b.revision),BotFleetConfiguration::summarize(a.revision.get(),*b.revision));}
TEST(PlayerBotFleetConfigurationTest,ActiveRevisionIsImmutable){auto r=BotFleetConfiguration::parse(fleetJson());static_assert(std::is_same_v<decltype(r.revision),std::shared_ptr<const BotFleetConfigurationRevision>>);ASSERT_TRUE(r.revision);}
TEST(PlayerBotFleetAdministrationTest,UnauthorizedCommandIsRejected){BotFleetAdministration admin;auto c=admin.submit({.type=BotFleetCommandType::Pause,.operatorId=9},false);EXPECT_EQ(BotFleetCommandFailure::Unauthorized,c.failure);EXPECT_EQ(0,admin.queued());}
TEST(PlayerBotFleetAdministrationTest,AcceptedIsDistinctFromCompleted){BotFleetAdministration admin;auto c=admin.submit({.type=BotFleetCommandType::Status,.operatorId=9},true);EXPECT_EQ(BotFleetCommandState::Accepted,c.state);EXPECT_EQ(1,admin.queued());}
TEST(PlayerBotFleetAdministrationTest,CommandQueueIsBounded){BotFleetAdministration admin;for(size_t i=0;i<BotFleetAdministration::MaximumQueuedCommands;++i)EXPECT_EQ(BotFleetCommandState::Accepted,admin.submit({.type=BotFleetCommandType::Status},true).state);EXPECT_EQ(BotFleetCommandFailure::QueueFull,admin.submit({.type=BotFleetCommandType::Status},true).failure);}
TEST(PlayerBotFleetAdministrationTest,AuditIsBoundedAndContainsNoCommandSecret){BotFleetAdministration admin;for(size_t i=0;i<BotFleetAdministration::MaximumAuditEntries+5;++i)(void)admin.submit({.type=BotFleetCommandType::Status,.reason="bounded"},false);EXPECT_EQ(BotFleetAdministration::MaximumAuditEntries,admin.audit().size());}
TEST(PlayerBotFleetAdministrationTest,AuditRedactsSecretLikeReasons){BotFleetAdministration admin;(void)admin.submit({.type=BotFleetCommandType::Status,.reason="token=hidden"},false);EXPECT_EQ("[redacted]",admin.audit().back().detail);}
TEST(PlayerBotFleetAdministrationTest,StopClearsCommandsAndRejectsNewWork){BotFleetAdministration admin;(void)admin.submit({.type=BotFleetCommandType::Status},true);admin.stop();EXPECT_EQ(0,admin.queued());EXPECT_EQ(BotFleetCommandFailure::Stopping,admin.submit({.type=BotFleetCommandType::Status},true).failure);}

TEST(PlayerBotFleetTelemetryTest, SnapshotContainsValuesOnly) { EXPECT_FALSE(BotFleetTelemetrySnapshot{}.containsWorldOwnership()); }
TEST(PlayerBotFleetTelemetryTest, FailuresAndEventsAreBounded) { BotFleetTelemetry telemetry; for(size_t i=0;i<BotFleetTelemetry::MaximumEvents+5;++i)telemetry.record(BotFleetTelemetryFailure::InvalidRequest,"bounded"); EXPECT_EQ(BotFleetTelemetry::MaximumEvents,telemetry.recent(BotFleetTelemetry::MaximumEvents+1).size()); }
TEST(PlayerBotFleetTelemetryTest, EventDetailsRedactSecrets) { BotFleetTelemetry telemetry;telemetry.record(BotFleetTelemetryFailure::InvalidRequest,"credential=hidden");ASSERT_EQ(1,telemetry.recent(1).size());EXPECT_EQ("[redacted]",telemetry.recent(1).front().detail); }
TEST(PlayerBotFleetTelemetryTest, DisabledTransportRejectsReads) { BotFleetAdministration admin; BotFleetTelemetry telemetry; BotFleetAdminService service(admin,telemetry); BotManager manager(g_game()); EXPECT_EQ(BotFleetTelemetryFailure::Disabled,service.handle(manager,{}).failure); }
TEST(PlayerBotFleetTelemetryTest, AuthenticationIsRequired) { BotFleetAdministration admin; BotFleetTelemetry telemetry; BotFleetAdminServicePolicy policy{.transport=BotFleetAdminTransport::Localhost}; BotFleetAdminService service(admin,telemetry,policy,[](uint64_t,std::string_view){return false;}); BotManager manager(g_game()); EXPECT_EQ(BotFleetTelemetryFailure::Unauthorized,service.handle(manager,{.encodedSize=1}).failure); }
TEST(PlayerBotFleetTelemetryTest, AuthenticatedStatusReturnsImmutableSnapshot) { BotFleetAdministration admin; BotFleetTelemetry telemetry; BotFleetAdminServicePolicy policy{.transport=BotFleetAdminTransport::UnixSocket}; BotFleetAdminService service(admin,telemetry,policy,[](uint64_t id,std::string_view secret){return id==7&&secret=="ok";}); BotManager manager(g_game()); auto response=service.handle(manager,{.operatorId=7,.encodedSize=2,.credential="ok"}); ASSERT_TRUE(response.success()); ASSERT_TRUE(response.snapshot); EXPECT_EQ(0,response.snapshot->managedSessions); }
TEST(PlayerBotFleetTelemetryTest, WritesEnqueueCommandsAndRemainAccepted) { BotFleetAdministration admin; BotFleetTelemetry telemetry; BotFleetAdminServicePolicy policy{.transport=BotFleetAdminTransport::Localhost}; BotFleetAdminService service(admin,telemetry,policy,[](uint64_t,std::string_view){return true;}); BotManager manager(g_game()); auto response=service.handle(manager,{.operation=BotFleetAdminOperation::Command,.operatorId=2,.encodedSize=4,.credential="ok",.command={.type=BotFleetCommandType::Pause}}); EXPECT_EQ(BotFleetCommandState::Accepted,response.command.state); EXPECT_EQ(1,admin.queued()); }
TEST(PlayerBotFleetTelemetryTest, RateAndRequestLimitsAreEnforced) { BotFleetAdministration admin; BotFleetTelemetry telemetry; BotFleetAdminServicePolicy policy{.transport=BotFleetAdminTransport::Localhost,.maximumRequestsPerWindow=1,.maximumRequestBytes=4}; BotFleetAdminService service(admin,telemetry,policy,[](uint64_t,std::string_view){return true;}); BotManager manager(g_game()); EXPECT_TRUE(service.handle(manager,{.window=1,.encodedSize=4}).success()); EXPECT_EQ(BotFleetTelemetryFailure::RateLimited,service.handle(manager,{.window=1,.encodedSize=4}).failure); EXPECT_EQ(BotFleetTelemetryFailure::RequestTooLarge,service.handle(manager,{.window=2,.encodedSize=5}).failure); }
TEST(PlayerBotFleetTelemetryTest, ExpiredAdminRequestDoesNotReachCommandQueue) { BotFleetAdministration admin; BotFleetTelemetry telemetry; BotFleetAdminServicePolicy policy{.transport=BotFleetAdminTransport::Localhost,.maximumRequestAgeTicks=5}; BotFleetAdminService service(admin,telemetry,policy,[](uint64_t,std::string_view){return true;}); BotManager manager(g_game()); auto response=service.handle(manager,{.operation=BotFleetAdminOperation::Command,.submittedAtTick=10,.handledAtTick=16,.encodedSize=1,.command={.type=BotFleetCommandType::Pause}}); EXPECT_EQ(BotFleetTelemetryFailure::TimedOut,response.failure); EXPECT_EQ(0,admin.queued()); ASSERT_EQ(1,telemetry.recent(1).size()); EXPECT_EQ(BotFleetTelemetryFailure::TimedOut,telemetry.recent(1).front().failure); }
TEST(PlayerBotFleetTelemetryTest, AuditAndEventReadsAreBounded) { BotFleetAdministration admin; for(size_t i=0;i<10;++i)(void)admin.submit({.type=BotFleetCommandType::Status},false); BotFleetTelemetry telemetry; for(size_t i=0;i<10;++i)telemetry.record(BotFleetTelemetryFailure::InvalidRequest); BotFleetAdminServicePolicy policy{.transport=BotFleetAdminTransport::Localhost,.maximumAuditResults=3,.maximumEventResults=2}; BotFleetAdminService service(admin,telemetry,policy,[](uint64_t,std::string_view){return true;}); BotManager manager(g_game()); EXPECT_EQ(3,service.handle(manager,{.operation=BotFleetAdminOperation::Audit,.encodedSize=1,.limit=20}).audit.size()); EXPECT_EQ(2,service.handle(manager,{.operation=BotFleetAdminOperation::Events,.window=1,.encodedSize=1,.limit=20}).events.size()); }
TEST(PlayerBotFleetTelemetryTest, ShutdownRejectsCallbacksAndClearsEvents) { BotFleetAdministration admin; BotFleetTelemetry telemetry; telemetry.record(BotFleetTelemetryFailure::InvalidRequest); BotFleetAdminServicePolicy policy{.transport=BotFleetAdminTransport::Localhost}; BotFleetAdminService service(admin,telemetry,policy,[](uint64_t,std::string_view){return true;}); service.stop(); telemetry.stop(); BotManager manager(g_game()); EXPECT_EQ(BotFleetTelemetryFailure::Stopping,service.handle(manager,{}).failure); EXPECT_TRUE(telemetry.recent(10).empty()); }
TEST(PlayerBotFleetTelemetryTest, LocalSoakAdapterRequiresPrivateDirectoryAndStopsCleanly) { auto path=std::filesystem::temp_directory_path()/fmt::format("canary-soak-adapter-{}",OTSYS_TIME());std::filesystem::create_directory(path);std::filesystem::permissions(path,std::filesystem::perms::owner_all,std::filesystem::perm_options::replace);BotManager manager(g_game());BotFleetAdministration admin;BotFleetTelemetry telemetry;BotFleetLocalSoakAdapter adapter(path,manager,admin,telemetry);ASSERT_TRUE(adapter.valid());adapter.sample({},1);EXPECT_TRUE(std::filesystem::is_regular_file(path/"fleet-snapshots.jsonl"));EXPECT_TRUE(std::filesystem::is_regular_file(path/"managed-lifecycle.jsonl"));EXPECT_TRUE(std::filesystem::is_regular_file(path/"ticks.jsonl"));std::ifstream snapshot(path/"fleet-snapshots.jsonl");std::string line;ASSERT_TRUE(std::getline(snapshot,line));EXPECT_NE(std::string::npos,line.find("managedMembers"));adapter.stop();EXPECT_FALSE(adapter.valid());std::filesystem::remove_all(path); }
TEST(PlayerBotFleetTelemetryTest, LocalSoakAdapterRejectsGroupAccessibleDirectory) { auto path=std::filesystem::temp_directory_path()/fmt::format("canary-soak-public-{}",OTSYS_TIME());std::filesystem::create_directory(path);std::filesystem::permissions(path,std::filesystem::perms::owner_all|std::filesystem::perms::group_read,std::filesystem::perm_options::replace);BotManager manager(g_game());BotFleetAdministration admin;BotFleetTelemetry telemetry;BotFleetLocalSoakAdapter adapter(path,manager,admin,telemetry);EXPECT_FALSE(adapter.valid());std::filesystem::remove_all(path); }
TEST(PlayerBotFleetTelemetryTest, LocalSoakAdapterRecordsAcceptedAndCompletedCommandSeparately) { auto path=std::filesystem::temp_directory_path()/fmt::format("canary-soak-command-{}",OTSYS_TIME());std::filesystem::create_directory(path);std::filesystem::permissions(path,std::filesystem::perms::owner_all,std::filesystem::perm_options::replace);{std::ofstream(path/"command-requests.jsonl")<<R"({"requestId":7,"operation":"status"})"<<'\n';}BotManager manager(g_game());BotFleetAdministration admin;BotFleetTelemetry telemetry;BotFleetLocalSoakAdapter adapter(path,manager,admin,telemetry);adapter.sample({},1);std::ifstream input(path/"commands.jsonl");std::string first,second;ASSERT_TRUE(std::getline(input,first));ASSERT_TRUE(std::getline(input,second));EXPECT_NE(std::string::npos,first.find("accepted"));EXPECT_NE(std::string::npos,second.find("completed"));adapter.stop();std::filesystem::remove_all(path); }
TEST(PlayerBotFleetTelemetryTest, LocalSoakAdapterRejectsDuplicateRequestId) { auto path=std::filesystem::temp_directory_path()/fmt::format("canary-soak-duplicate-{}",OTSYS_TIME());std::filesystem::create_directory(path);std::filesystem::permissions(path,std::filesystem::perms::owner_all,std::filesystem::perm_options::replace);{std::ofstream output(path/"command-requests.jsonl");output<<R"({"requestId":7,"operation":"status"})"<<'\n'<<R"({"requestId":7,"operation":"status"})"<<'\n';}BotManager manager(g_game());BotFleetAdministration admin;BotFleetTelemetry telemetry;BotFleetLocalSoakAdapter adapter(path,manager,admin,telemetry);adapter.sample({},1);std::ifstream input(path/"commands.jsonl");std::string all((std::istreambuf_iterator<char>(input)),{});EXPECT_NE(std::string::npos,all.find("duplicate_request_id"));adapter.stop();std::filesystem::remove_all(path); }

namespace { BotFleetResourcePolicy resourcePolicy(){return{.maximumManagedBots=64,.maximumPendingLogins=8,.maximumPendingLogouts=8,.maximumCommandsQueued=64,.maximumPlannerActionsPerTick=64,.maximumReconciliationWorkPerTick=64,.maximumDispatcherBacklog=100,.maximumSchedulerBacklog=100,.recoveryObservations=2};} }
TEST(PlayerBotFleetHardeningTest,ResourceContractsContainValuesOnly){static_assert(std::is_trivially_copyable_v<BotFleetResourceObservation>);static_assert(std::is_trivially_copyable_v<BotFleetLoadSheddingDecision>);SUCCEED();}
TEST(PlayerBotFleetHardeningTest,ResourcePolicyEnforcesAbsoluteLimits){auto p=resourcePolicy();EXPECT_TRUE(BotFleetHardening::validate(p));p.maximumCommandsQueued=65;EXPECT_FALSE(BotFleetHardening::validate(p));}
TEST(PlayerBotFleetHardeningTest,NormalPressurePermitsBoundedWork){auto d=BotFleetHardening::observe(resourcePolicy(),{});EXPECT_EQ(BotFleetPressureState::Normal,d.pressure);EXPECT_TRUE(d.permitBotLogin);EXPECT_EQ(64,d.reconciliationWork);}
TEST(PlayerBotFleetHardeningTest,ElevatedPressureReducesOptionalWork){BotFleetResourceObservation o{.dispatcherBacklog=51};auto d=BotFleetHardening::observe(resourcePolicy(),o);EXPECT_EQ(BotFleetPressureState::Elevated,d.pressure);EXPECT_FALSE(d.permitPlannerExpansion);EXPECT_LT(d.reconciliationWork,64);}
TEST(PlayerBotFleetHardeningTest,OverloadBlocksLoginAndPermitsSafeDrain){BotFleetResourceObservation o{.dispatcherBacklog=101};auto d=BotFleetHardening::observe(resourcePolicy(),o);EXPECT_EQ(BotFleetPressureState::Overloaded,d.pressure);EXPECT_FALSE(d.permitBotLogin);EXPECT_TRUE(d.permitSafeDrain);EXPECT_TRUE(d.preserveSaveAndLogout);}
TEST(PlayerBotFleetHardeningTest,CriticalPressureProtectsOrdinaryPlayer){BotFleetResourceObservation o{.ordinaryPlayerResponsive=false};auto d=BotFleetHardening::observe(resourcePolicy(),o);EXPECT_EQ(BotFleetPressureState::Critical,d.pressure);EXPECT_FALSE(d.permitCoordinationWork);EXPECT_FALSE(d.permitBotLogin);}
TEST(PlayerBotFleetHardeningTest,RecoveryUsesHysteresis){EXPECT_EQ(BotFleetPressureState::Recovering,BotFleetHardening::observe(resourcePolicy(),{},BotFleetPressureState::Overloaded,1).pressure);EXPECT_EQ(BotFleetPressureState::Normal,BotFleetHardening::observe(resourcePolicy(),{},BotFleetPressureState::Recovering,2).pressure);}
TEST(PlayerBotFleetHardeningTest,HardPendingLoginMaximumIsCritical){BotFleetResourceObservation o{.pendingLogins=9};EXPECT_EQ(BotFleetPressureState::Critical,BotFleetHardening::observe(resourcePolicy(),o).pressure);}
TEST(PlayerBotFleetHardeningTest,HardPendingLogoutMaximumIsCritical){BotFleetResourceObservation o{.pendingLogouts=9};EXPECT_EQ(BotFleetPressureState::Critical,BotFleetHardening::observe(resourcePolicy(),o).pressure);}
TEST(PlayerBotFleetHardeningTest,HardCommandQueueMaximumIsCritical){BotFleetResourceObservation o{.commandsQueued=65};EXPECT_EQ(BotFleetPressureState::Critical,BotFleetHardening::observe(resourcePolicy(),o).pressure);}
TEST(PlayerBotFleetHardeningTest,ProfilesAreDeterministicAndConservative){EXPECT_EQ(BotFleetHardening::profile(BotFleetScaleProfile::Medium),BotFleetHardening::profile(BotFleetScaleProfile::Medium));EXPECT_EQ(2,BotFleetHardening::profile(BotFleetScaleProfile::Smoke).desiredPopulation);}
TEST(PlayerBotFleetHardeningTest,SoakTickBudgetPassesAtExactTerminalTick){auto p=BotFleetHardening::profile(BotFleetScaleProfile::Smoke);std::vector<BotFleetSoakObservation>o;for(uint32_t i=1;i<=p.tickBudget;++i)o.push_back({.tick=i,.placed=2,.uniqueSessions=2});auto r=BotFleetHardening::run(p,o);EXPECT_TRUE(r.passed);EXPECT_EQ(100,r.ticksCompleted);EXPECT_EQ(10,r.summaries);}
TEST(PlayerBotFleetHardeningTest,IncompleteSoakFails){auto p=BotFleetHardening::profile(BotFleetScaleProfile::Smoke);auto r=BotFleetHardening::run(p,{{.tick=99}});EXPECT_FALSE(r.passed);EXPECT_EQ(BotFleetReleaseBlocker::SoakIncomplete,r.blocker);}
TEST(PlayerBotFleetHardeningTest,DuplicateSessionTerminatesSoak){auto p=BotFleetHardening::profile(BotFleetScaleProfile::Smoke);auto r=BotFleetHardening::run(p,{{.tick=1,.placed=2,.uniqueSessions=1}});EXPECT_EQ(BotFleetReleaseBlocker::InvariantFailure,r.blocker);}
TEST(PlayerBotFleetHardeningTest,CallbackAfterDestructionTerminatesSoak){auto p=BotFleetHardening::profile(BotFleetScaleProfile::Smoke);auto r=BotFleetHardening::run(p,{{.tick=1,.callbacksAfterDestruction=1}});EXPECT_EQ(BotFleetReleaseBlocker::InvariantFailure,r.blocker);}
TEST(PlayerBotFleetHardeningTest,OrdinaryPlayerStarvationTerminatesSoak){auto p=BotFleetHardening::profile(BotFleetScaleProfile::Smoke);auto r=BotFleetHardening::run(p,{{.tick=1,.ordinaryPlayerResponsive=false}});EXPECT_EQ(BotFleetReleaseBlocker::OrdinaryPlayerStarvation,r.blocker);}
TEST(PlayerBotFleetHardeningTest,FaultResultsAreNormalizedAndBounded){auto r=BotFleetHardening::fault(BotFleetFaultScenario::DatabaseFailureDuringSave,3,true);EXPECT_EQ(BotFleetFailure::RetryExhausted,r.normalizedFailure);EXPECT_TRUE(r.bounded);EXPECT_TRUE(r.recovered);}
TEST(PlayerBotFleetHardeningTest,PressureFaultsNormalizeToBudget){EXPECT_EQ(BotFleetFailure::BudgetReached,BotFleetHardening::fault(BotFleetFaultScenario::DispatcherPressure,1,true).normalizedFailure);}
TEST(PlayerBotFleetHardeningTest,AuthenticationFailureNormalizesWithoutSecret){auto r=BotFleetHardening::fault(BotFleetFaultScenario::AuthenticationFailure,1,false);EXPECT_EQ(BotFleetFailure::InvalidPolicy,r.normalizedFailure);}
TEST(PlayerBotFleetHardeningTest,IdenticalStreamsProduceIdenticalDecisions){auto p=resourcePolicy();BotFleetResourceObservation o{.schedulerBacklog=101};EXPECT_EQ(BotFleetHardening::observe(p,o),BotFleetHardening::observe(p,o));}
TEST(PlayerBotFleetHardeningTest,SupportedDispatcherCategoryTriggersCriticalPressure){BotFleetResourceObservation o{.dispatcherPressure=BotFleetPressureState::Critical};auto d=BotFleetHardening::observe(resourcePolicy(),o);EXPECT_EQ(BotFleetPressureState::Critical,d.pressure);EXPECT_FALSE(d.permitBotLogin);}
TEST(PlayerBotFleetHardeningTest,ResourceConfigurationCannotUndercutFleetBounds){BotManager manager(g_game());BotFleetPopulationPolicy population{.desiredOnline=0,.minimumOnline=0,.maximumOnline=65,.absoluteHardMaximum=65,.maximumPendingLogins=1,.maximumPendingLogouts=1,.revision=1};EXPECT_EQ(BotFleetFailure::InvalidPolicy,manager.configureFleet(population,{},{}));}
