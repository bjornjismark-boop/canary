...
BotLootTransferResult BotController::executeLoot(const BotLootTransferRequest &request, std::chrono::milliseconds now, const BotLootPolicy &lootPolicy, const BotLootTransferPolicy &policy) {
	...
	if ((lootProgress.state == BotLootExecutionState::Completed || lootProgress.state == BotLootExecutionState::Failed) && lootProgress.request && *lootProgress.request != request) lootProgress = {};
...
BotDepotObservation BotController::observeDepot(uint32_t depotId,const BotResupplyPolicy &policy){const auto controlledPlayer=player.lock();if(!controlledPlayer||controlledPlayer->isRemoved()||!controlledPlayer->getTile())return {};return BotResupply::observeDepot(controlledPlayer,depotId,policy);}
BotResupplyResult BotController::executeResupply(const BotResupplyRequest &request,std::chrono::milliseconds now,const BotResupplyPolicy &policy){const auto controlledPlayer=player.lock();if(!controlledPlayer||controlledPlayer->isRemoved()||!controlledPlayer->getTile())return{.outcome=BotResupplyOutcome::Cancelled,.state=BotResupplyState::Failed,.request=request};if(resupplyProgress.transfer||resupplyProgress.equipment)return{.outcome=BotResupplyOutcome::Pending,.state=resupplyProgress.state,.request=request};resupplyProgress={.state=BotResupplyState::TransferPending,.transfer=request,.startedAt=now};auto result=BotResupply::executeTransfer(game,controlledPlayer,request,policy);resupplyProgress={.state=result.state};return result;}
BotEquipmentExecutionResult BotController::executeEquipment(const BotEquipmentExecutionRequest &request,std::chrono::milliseconds now,const BotEquipmentPolicy &policy){const auto controlledPlayer=player.lock();if(!controlledPlayer||controlledPlayer->isRemoved()||!controlledPlayer->getTile())return{.outcome=BotResupplyOutcome::Cancelled,.failure=BotEquipmentFailure::InvalidLifecycle,.state=BotResupplyState::Failed,.request=request};if(resupplyProgress.transfer||resupplyProgress.equipment)return{.outcome=BotResupplyOutcome::Pending,.state=resupplyProgress.state,.request=request};resupplyProgress={.state=BotResupplyState::Equipping,.equipment=request,.startedAt=now};auto result=BotResupply::executeEquipment(game,controlledPlayer,request,policy);resupplyProgress={.state=result.state};return result;}
BotResupplyResult BotController::cancelResupply(){resupplyProgress={.state=BotResupplyState::Cancelled};return{.outcome=BotResupplyOutcome::Cancelled,.state=BotResupplyState::Cancelled};}

BotDialogueObservation BotController::observeDialogue(const BotNpcDialoguePolicy &policy){const auto controlled=player.lock();if(!controlled||controlled->isRemoved()||!controlled->getTile())return{};return BotDialogue::observe(controlled,policy);}
#define TALKTYPE_PRIVATE_PN TALKTYPE_SAY
BotConversationResult BotController::advanceDialogue(uint32_t npcId,BotDialogueIntent intent,std::chrono::milliseconds now,const BotNpcDialoguePolicy &policy){BotConversationResult result{.npcId=npcId,.intent=intent};const auto controlled=player.lock();if(!controlled||controlled->isRemoved()||!controlled->getTile()){result.state=BotDialogueState::Failed;result.failure=BotDialogueFailure::InvalidLifecycle;return result;}if(intent==BotDialogueIntent::Cancel)return cancelDialogue();const auto observation=BotDialogue::observe(controlled,policy);const auto npc=std::ranges::find(observation.npcs,npcId,&BotNpcObservation::npcId);if(npc==observation.npcs.end()){dialogueProgress={.state=BotDialogueState::ConversationLost};result.state=BotDialogueState::ConversationLost;result.response=BotDialogueResponse::NpcUnavailable;result.failure=BotDialogueFailure::NpcUnavailable;return result;}if(!npc->sameFloor||npc->distance>4){dialogueProgress={.state=BotDialogueState::ConversationLost};result.state=BotDialogueState::ConversationLost;result.response=BotDialogueResponse::OutOfRange;result.failure=npc->sameFloor?BotDialogueFailure::OutOfRange:BotDialogueFailure::DifferentFloor;return result;}const auto phrase=BotDialogue::phrase(policy,intent);if(!phrase){result.state=BotDialogueState::Failed;result.failure=BotDialogueFailure::UnconfiguredPhrase;return result;}if(dialogueProgress.state==BotDialogueState::AwaitingResponse&&dialogueProgress.npcId==npcId&&dialogueProgress.intent==intent){const auto response=BotDialogue::response(controlled,npcId,dialogueProgress.speechRevision,*phrase,policy.maximumResponseLength);result.phrasesSent=dialogueProgress.phrasesSent;result.attempts=dialogueProgress.attempts;result.responseRevision=response.revision;result.responseTokenHash=response.normalizedTokenHash;if(response.category!=BotDialogueResponse::None){result.response=response.category;result.state=response.category==BotDialogueResponse::ConversationComplete?BotDialogueState::ConversationComplete:BotDialogueState::TopicActive;dialogueProgress={.state=result.state,.npcId=npcId,.phrasesSent=result.phrasesSent,.attempts=result.attempts,.speechRevision=response.revision};return result;}if(now-dialogueProgress.sentAt<policy.responseWait){result.state=BotDialogueState::AwaitingResponse;result.response=BotDialogueResponse::NoResponse;return result;}if(dialogueProgress.attempts>=policy.maximumRetries){dialogueProgress={.state=BotDialogueState::TimedOut};result.state=BotDialogueState::TimedOut;result.response=BotDialogueResponse::RetryExhausted;result.failure=BotDialogueFailure::RetryExhausted;return result;}dialogueProgress.state=BotDialogueState::Backoff;dialogueProgress.nextAttemptAt=now+BotDialogue::backoff(policy,dialogueProgress.attempts);result.state=BotDialogueState::Backoff;result.response=BotDialogueResponse::RetryScheduled;result.failure=BotDialogueFailure::TimedOut;return result;}if(dialogueProgress.state==BotDialogueState::Backoff){if(now<dialogueProgress.nextAttemptAt){result.state=BotDialogueState::Backoff;result.response=BotDialogueResponse::RetryScheduled;return result;}++dialogueProgress.attempts;}else dialogueProgress={.state=BotDialogueState::NpcSelected,.npcId=npcId};if(dialogueProgress.phrasesSent>=policy.maximumPhrases){result.state=BotDialogueState::Failed;result.failure=BotDialogueFailure::PhraseLimit;return result;}const auto speechRevision=controlled->getVisibleSpeechRevision();game.playerSay(controlled->getID(),0,TALKTYPE_PRIVATE_PN,"",phrase->text);dialogueProgress.state=BotDialogueState::AwaitingResponse;dialogueProgress.intent=intent;dialogueProgress.speechRevision=speechRevision;dialogueProgress.sentAt=now;++dialogueProgress.phrasesSent;++dialogueProgress.attempts;result.state=BotDialogueState::AwaitingResponse;result.response=BotDialogueResponse::NoResponse;result.phrasesSent=dialogueProgress.phrasesSent;result.attempts=dialogueProgress.attempts;return result;}
...
BotQuestExecutionResult BotController::startQuestExecution(const BotQuestPlan&plan,const BotQuestExecutionPolicy&policy){if(!questObservation)return{.state=BotQuestExecutionState::Failed,.failure=BotQuestExecutionFailure::ObservationStale};const auto mission=std::ranges::find(questObservation->missions,plan.missionId,&BotMissionObservation::missionId);if(mission==questObservation->missions.end())return{.state=BotQuestExecutionState::Failed,.failure=BotQuestExecutionFailure::InvalidPlan};BotQuestEligibility eligibility;eligibility.result=mission->state==BotMissionState::Unavailable||mission->state==BotMissionState::Stale?BotQuestFailure::PolicyRejected:BotQuestFailure::Eligible;questExecution=BotQuestExecution::start(plan,*questObservation,eligibility,policy);return *questExecution;}
BotQuestExecutionResult BotController::advanceQuestExecution(const BotQuestPlan&plan,const BotQuestStepObservation&observation,const BotQuestExecutionPolicy&policy){if(!questExecution)return{.state=BotQuestExecutionState::Failed,.failure=BotQuestExecutionFailure::InvalidLifecycle};questExecution=BotQuestExecution::advance(plan,*questExecution,observation,policy);return *questExecution;}
