...
BotPlannerMigration BotPlannerPersistence::migrate(BotPersistedPlanCheckpoint c, uint32_t expectedPlayerId) {
	if (c.checksum != checksum(c)) return { .result = BotPlannerMigrationResult::Corrupt };
	if (c.playerId == 0 || expectedPlayerId == 0) return { .result = BotPlannerMigrationResult::MissingRequiredField };
	if (c.playerId != expectedPlayerId) return { .result = BotPlannerMigrationResult::PlayerMismatch };
	if (c.schemaVersion == 2) return { .result = BotPlannerMigrationResult::Current, .checkpoint = c, .freshObservationRequired = true };
	if (c.schemaVersion == 0) return { .result = BotPlannerMigrationResult::UnsupportedOlderVersion };
	if (c.schemaVersion > 2) return { .result = BotPlannerMigrationResult::UnsupportedFutureVersion };
	if (c.schemaVersion != 1 || c.policyRevision == 0 || c.goalId == 0 || c.planRevision == 0 || !c.safeSaveBoundary) return { .result = BotPlannerMigrationResult::Rejected };
	c.schemaVersion = 2;
	c.failureCount = std::min<uint8_t>(c.failureCount, 16);
	c.retryCount = std::min<uint8_t>(c.retryCount, 16);
	c.checksum = checksum(c);
	return { .result = BotPlannerMigrationResult::Migrated, .checkpoint = c, .freshObservationRequired = true, .migratedFields = 3 };
}

BotCheckpointLoadResult BotPlannerPersistence::validate(BotPersistedPlanCheckpoint c, const BotCheckpointValidation &v) {
	auto reject=[](BotCheckpointLoadReason reason){BotCheckpointLoadResult result;result.reason=reason;return result;};
	if (c.schemaVersion == 1 && v.schemaVersion == 1) {
		auto migrated = migrate(c, v.playerId);
		if (migrated.result != BotPlannerMigrationResult::Migrated) return reject(BotCheckpointLoadReason::UnsupportedVersion);
		c = *migrated.checkpoint;
	} else if (c.schemaVersion != v.schemaVersion && !(c.schemaVersion == 2 && v.schemaVersion == 1)) return reject(BotCheckpointLoadReason::UnsupportedVersion);
	if(c.checksum!=checksum(c)||c.playerId==0||c.checkpointRevision==0||c.policyRevision==0||c.goalId==0||c.planRevision==0||c.goalType>BotGoalType::Unsupported||c.verifiedSubsystem>BotPlanSubsystem::Lifecycle)return reject(BotCheckpointLoadReason::Corrupt);
	if(c.playerId!=v.playerId)return reject(BotCheckpointLoadReason::PlayerMismatch);
	if(c.policyRevision!=v.policyRevision)return reject(BotCheckpointLoadReason::PolicyRevisionChanged);
	if(!std::ranges::contains(v.goalIds,c.goalId))return reject(BotCheckpointLoadReason::GoalUnavailable);
	if(!std::ranges::contains(v.planRevisions,c.planRevision))return reject(BotCheckpointLoadReason::PlanMissing);
	if(c.verifiedStepIndex>v.planStepCount)return reject(BotCheckpointLoadReason::StepOutOfRange);
	if(!c.safeSaveBoundary)return reject(BotCheckpointLoadReason::UnsafeBoundary);
	if(!v.observationFresh)return reject(BotCheckpointLoadReason::ObservationRequired);
	if(!v.postconditionValid)return reject(BotCheckpointLoadReason::PostconditionInvalid);
	return{.reason=BotCheckpointLoadReason::Valid,.checkpoint=c,.freshObservationRequired=false,.durable=false};
}
