...
struct BotCheckpointValidation { uint32_t playerId = 0; uint16_t schemaVersion = 1; uint64_t policyRevision = 1; std::vector<BotGoalId> goalIds; std::vector<uint64_t> planRevisions; uint16_t planStepCount = 0; bool postconditionValid = false; bool observationFresh = false; };
